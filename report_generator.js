...
function generateMiniProjectReport_Part1() {
  // 1. Copy template doc
  ...
  Logger.log("Report (Part 1) generated with ID: " + newDocId);
}
...
const REPORT_ID = "1Hb1luKmM7AwZRvm7JAzuf09wB_7dtXsEgTHuvzbSM2w";

/********************************************
 * PART 2: TOC + LISTS + CHAPTER 1
 ********************************************/

function generateMiniProjectReport_Part2() {
  const doc = DocumentApp.openById(REPORT_ID);
  const body = doc.getBody();

  // Append after existing content (Cover, Certificate, Acknowledgement, Synopsis)
  body.appendPageBreak();
  ...
  doc.saveAndClose();
  Logger.log("Part 2 content (TOC + Lists + Chapter 1) added to report: " + REPORT_ID);
}
...
function generateMiniProjectReport_Part3() {
  const doc = DocumentApp.openById(REPORT_ID);
  const body = doc.getBody();
  ...
  doc.saveAndClose();
  Logger.log("Part 3 content (Chapter 2 + 3) added to report: " + REPORT_ID);
}
...
function writeChapter3_SRS(body) {
  ...
  p = body.appendParagraph("SOFTWARE REQUIREMENTS SPECIFICATION (SRS)");
  p.setFontFamily("Times New Roman")
    .setFontSize(14)
    .setBold(true)
    .setAlignment(DocumentApp.HorizontalAlignment.CENTER);
...
function generateMiniProjectReport_Part4() {
  const doc = DocumentApp.openById(REPORT_ID);
  const body = doc.getBody();
  ...
  doc.saveAndClose();
  Logger.log("Part 4 content (Chapter 4) added to report: " + REPORT_ID);
}
...
function writeChapter4_SystemDesign(body) {
  ...
  var text2 =
    "This is the core logic of the system, built using Python Flask. It includes:\n" +
...
function generateMiniProjectReport_Part5() {
  const doc = DocumentApp.openById(REPORT_ID);
  const body = doc.getBody();
  ...
  doc.saveAndClose();
  Logger.log("Part 5 content (Chapter 5) added to report: " + REPORT_ID);
}
...
function writeChapter5_ModulesAndDB(body) {
  ...
  text =
    "This module is responsible for creating and maintaining records of faculty members. It allows the timetable " +
    "coordinator to add, update, and delete faculty details such as name, faculty code, department, maximum teaching " +
    "load, and unavailable time slots. The data maintained by this module is used by the scheduler to ensure that " +
    "no faculty member is assigned overlapping classes or assigned beyond the permitted workload.";
  body.appendParagraph(text)
    .setFontFamily("Times New Roman")
    .setFontSize(12)
    .setAlignment(DocumentApp.HorizontalAlignment.JUSTIFY);
  ...
  var dbRows = [
    ...
    [
      ...
      "Stores the final allocation of a course (via mapping) to a room and time slot."
    ]
  ];

  for (var r = 0; r < dbRows.length; r++) {
    var row = table.appendTableRow();
    for (var c = 0; c < dbRows[r].length; c++) {
      row.appendTableCell(dbRows[r][c]);
    }
  }
  ...
  /*************** 5.4 SUMMARY ***************/
  p = body.appendParagraph("5.4 Summary");
  p.setFontFamily("Times New Roman")
    .setFontSize(13)
    .setBold(true)
    .setHeading(DocumentApp.ParagraphHeading.HEADING2);
  ...
  text =
    "In this chapter, the internal structure of the Automatic Timetable Generator has been discussed in detail. " +
    "The modular decomposition of the system into functional blocks and the corresponding database tables ensures " +
    "that the system is scalable, maintainable, and easy to extend. The next chapter focuses on the implementation " +
    "details and snapshots of the developed web application.";
  body.appendParagraph(text)
    .setFontFamily("Times New Roman")
    .setFontSize(12)
    .setLineSpacing(1.5)
...
/********************************************
 * PART 6: CHAPTER 6, 7, 8 + CONCLUSION & REFERENCES
 ********************************************/

function generateMiniProjectReport_Part6() {
  const doc = DocumentApp.openById(REPORT_ID);
  const body = doc.getBody();
  ...
  doc.saveAndClose();
  Logger.log("Part 6 content (Ch 6,7,8 + Conclusion & References) added to report: " + REPORT_ID);
}
...
function writeConclusionAndReferences(body) {
  ...
  var p = body.appendParagraph("CONCLUSION");
  p.setAlignment(DocumentApp.HorizontalAlignment.CENTER)
    .setFontFamily("Times New Roman")
    .setFontSize(16)
    .setBold(true)
